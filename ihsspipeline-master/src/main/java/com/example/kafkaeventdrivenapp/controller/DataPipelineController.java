...
import com.example.kafkaeventdrivenapp.service.FieldVisibilityService;
import com.example.kafkaeventdrivenapp.service.ReportGenerationService;
import com.example.kafkaeventdrivenapp.service.RulesEngineService;
import com.example.kafkaeventdrivenapp.service.QueryBuilderService;
import com.example.kafkaeventdrivenapp.service.DataFetchingService;
...
public class DataPipelineController {
    ...
    public ResponseEntity<Map<String, Object>> extractDataEnhanced(@RequestBody PipelineExtractionRequest request) {
    ...
    public ResponseEntity<Map<String, Object>> getReportTypes() {
        ...
        try {
            ...
            List<String> reportTypes = reportTypeProperties.getAll();
    ...
    /**
     * Get field visibility information for a specific role (JWT-ONLY)
     */
    ...
    public ResponseEntity<Map<String, Object>> getFieldVisibility(@PathVariable String userRole) {
        ...
        try {
            ...
            if (jwtToken == null) {
                response.put("status", "ERROR");
                response.put("message", "JWT token is required for field visibility");
                return ResponseEntity.badRequest().body(response);
            }
            
            Map<String, Object> visibilitySummary = fieldVisibilityService.getFieldVisibilitySummary(userRole, jwtToken);
            response.put("status", "SUCCESS");
    ...
    /**
     * Get all available fields for a specific role (JWT-ONLY)
     * Note: Changed from "all roles" to "specific role" since JWT is required
     */
    @GetMapping("/available-fields")
    public ResponseEntity<Map<String, Object>> getAvailableFields(@RequestParam(required = false) String userRole) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            // Extract user role from JWT if not provided
            if (userRole == null || userRole.trim().isEmpty()) {
                Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
                if (authentication != null && authentication.getPrincipal() instanceof Jwt) {
                    Jwt jwt = (Jwt) authentication.getPrincipal();
                    Map<String, Object> userInfo = extractUserInfoFromJwtObject(jwt);
            ...
            List<String> availableFields = fieldVisibilityService.getAllAvailableFields(userRole, jwtToken);
            response.put("status", "SUCCESS");
            ...
            for (String role : roles) {
                try {
                    ...
                    if (roleData != null && "SUCCESS".equals(roleData.get("status"))) {
                        ...
                        // Get field visibility information for this role (requires JWT)
                        try {
                            String jwtToken = extractJwtTokenFromSecurityContext();
                            if (jwtToken != null) {
                                Map<String, Object> fieldVisibility = fieldVisibilityService.getFieldVisibilitySummary(role, jwtToken);
                                ...
                                System.err.println("⚠️ JWT token not available, skipping field visibility for role " + role);
                            }
                        } catch (Exception e) {
                        ...
                        if (dataObj != null) {
                            try {
                                ...
                                if (dataObj instanceof com.example.kafkaeventdrivenapp.model.ReportData) {
                                    com.example.kafkaeventdrivenapp.model.ReportData reportData = (com.example.kafkaeventdrivenapp.model.ReportData) dataObj;
    ...
    /**
     * Extract JWT token string from Spring Security Context
     */
    private String extractJwtTokenFromSecurityContext() {
        try {
            ...
            System.err.println("⚠️ DataPipelineController: Error extracting JWT token from SecurityContext: " + e.getMessage());
        }
        return null;
    }
    ...
    private Map<String, Object> extractUserInfoFromJwtObject(Jwt jwt) {
        ...
        try {
            ...
            if (countyId != null && !countyId.trim().isEmpty()) {
                ...
                // Try county_id (with underscore) - protocol mapper creates this claim
                countyId = jwt.getClaimAsString("county_id");
                System.out.println("🔍 DataPipelineController: Direct county_id claim: " + countyId);
                if (countyId != null && !countyId.trim().isEmpty()) {
                    userInfo.put("countyId", countyId);
                    System.out.println("✅ DataPipelineController: Found county_id directly in JWT object (mapped to countyId): " + countyId);
                } else {
                // Try attributes.countyId
    ...
    private Map<String, Object> extractUserInfoFromJWT(String jwtToken) {
        ...
        try {
            ...
            if (jsonNode.has("countyId")) {
                String directCountyId = jsonNode.get("countyId").asText();
                userInfo.put("countyId", directCountyId);
                System.out.println("✅ DataPipelineController: Found countyId directly in JWT: " + directCountyId);
            } else if (jsonNode.has("county_id")) {
                // Fallback: check for county_id (with underscore) - protocol mapper creates this claim
                String directCountyId = jsonNode.get("county_id").asText();
                userInfo.put("countyId", directCountyId);
                System.out.println("✅ DataPipelineController: Found county_id directly in JWT (mapped to countyId): " + directCountyId);
                ...
                if (attributes.has("countyId")) {
                    ...
                    if (countyIdNode.isArray() && countyIdNode.size() > 0) {
                        String countyId = countyIdNode.get(0).asText();
                        userInfo.put("countyId", countyId);
                        System.out.println("✅ DataPipelineController: Extracted countyId from attributes array: " + countyId);
                    } else if (countyIdNode.isTextual()) {
                        String countyId = countyIdNode.asText();
                        userInfo.put("countyId", countyId);
                        System.out.println("✅ DataPipelineController: Extracted countyId from attributes text: " + countyId);
                    }
                } else {
                    System.out.println("⚠️ DataPipelineController: 'attributes' object exists but no 'countyId' field found");
