...
public interface ReportJobRepository extends JpaRepository<ReportJobEntity, String> {
    ...
    // Find jobs by report type, user role, and status (for dependency checking)
    List<ReportJobEntity> findByReportTypeAndUserRoleAndStatus(String reportType, String userRole, String status);
    
    // Find jobs by multiple report types, user role, and status (for multiple dependencies)
    @Query("SELECT j FROM ReportJobEntity j WHERE j.reportType IN :reportTypes AND j.userRole = :userRole AND j.status = :status")
    List<ReportJobEntity> findByReportTypeInAndUserRoleAndStatus(@Param("reportTypes") List<String> reportTypes, 
